...
class PushNotification(DefinedMessage):
    ...
    retrieved using the
    [PushNotification.find_avps][diameter.message.Message.find_avps]
    search method.
    ...
    Examples:
        ...
        either an instance of `PushNotificationRequest` or
        `PushNotificationAnswer` automatically:

        >>> msg = Message.from_bytes(b"...")
        >>> assert msg.header.is_request is True
        >>> assert isinstance(msg, PushNotificationRequest)

        When creating a new message, the `PushNotificationRequest` or
        `PushNotificationAnswer` class should be instantiated directly,
    ...
    [PushNotification.append_avp][diameter.message.Message.append_avp]
