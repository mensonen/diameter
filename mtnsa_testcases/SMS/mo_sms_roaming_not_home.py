...
# SCENARIO SELECTION
# ============================================================
SCENARIO_KEY = "mo_sms_international_roaming_third_country"

TEST_SCENARIOS = {
    "mo_sms_international_roaming_third_country": {
        "scenario_name": "MO SMS INTERNATIONAL ROAMING - SIM IN ROAMING COUNTRY - DESTINATION THIRD COUNTRY",
        "description": "SIM in Roaming Country; destination Not Home Country",
        "session_suffix": "sms-international-roaming-third-country",
        ...
        "type": "International",
        "b_party_msisdn": "919876543210",   # Replace with actual third-country destination
        "traffic_case": 20,

        "other_party_id_type": 0,
        "other_party_id_nature": 1,

        "enable_ims_information": True,
        "ims_node_functionality": 0,
        "ims_number_portability_routing_info": "D082",  # Replace if your routing map differs

        "enable_tgpp_mcc_mnc_avps": True,
        "tgpp_imsi_mcc_mnc": "23410",       # Roaming source MCCMNC
        "tgpp_ggsn_mcc_mnc": "40410",       # Third-country destination MCCMNC
        "tgpp_sgsn_mcc_mnc": "23410",       # Serving roaming network MCCMNC

        "expected_source_mccmnc": "23410",
        "expected_b_leg": "MOSMS International",
    },
}
...
def validate_international_roaming(events):
    print_banner("VERIFY INTERNATIONAL ROAMING SMS")
    ...
    source_candidates = [imsi_mccmnc, sgsn_mccmnc]
    if str(CFG["expected_source_mccmnc"]) not in [str(x) for x in source_candidates]:
        raise AssertionError(
            ...
            f"got IMSI={imsi_mccmnc}, SGSN={sgsn_mccmnc}"
        )

    b_leg = deep_find_first(
        found_event,
        {"b_leg", "bleg", "additional_b_leg", "additional_bleg", "destination_leg_classification"}
    )

    print(f"Expected Additional B-Leg   : {CFG['expected_b_leg']}")
    #print(f"Kinesis Additional B-Leg    : {b_leg}")

    if b_leg is not None and str(b_leg) != str(CFG["expected_b_leg"]):
        raise AssertionError(
            f"Expected additional B-leg {CFG['expected_b_leg']}, got {b_leg}"
        )
...
def main():
    ...
    try:
        with socket.create_connection((SERVER_FQDN, SERVER_PORT), timeout=10) as sock:
            ...
            print("\nSTEP 2: Send MO SMS to third-country destination")
        ...
        validate_international_roaming(events)
