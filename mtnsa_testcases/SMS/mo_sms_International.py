...
# SCENARIO SELECTION
# ============================================================
SCENARIO_KEY = "mo_sms_international_home_network"

TEST_SCENARIOS = {
    "mo_sms_international_home_network": {
        "scenario_name": "MO SMS INTERNATIONAL - SIM ON HOME NETWORK - DESTINATION NOT HOME COUNTRY",
        "description": "SIM on Home Network; destination is Not Home Country",
        "session_suffix": "sms-international-home-network",
        ...
        "type": "International",
        "b_party_msisdn": "919876543210",   # Replace with actual international number
        "traffic_case": 20,

        "other_party_id_type": 0,
        "other_party_id_nature": 1,         # International confirmed earlier

        "enable_ims_information": True,
        "ims_node_functionality": 0,
        "ims_number_portability_routing_info": "D081",  # Replace if your platform expects different routing code

        "enable_tgpp_mcc_mnc_avps": True,
        "tgpp_imsi_mcc_mnc": "65510",       # Sender MCCMNC
        "tgpp_ggsn_mcc_mnc": "40410",       # Destination MCCMNC expected charging dimension
        "tgpp_sgsn_mcc_mnc": "65510",       # SIM still on home network

        "expected_destination_mccmnc": "40410",
    },
}
...
def validate_international_home_network(events):
    print_banner("VERIFY INTERNATIONAL HOME NETWORK CHARGING")
...
def main():
    ...
    try:
        with socket.create_connection((SERVER_FQDN, SERVER_PORT), timeout=10) as sock:
            ...
            print("\nSTEP 2: Send MO SMS to international number")
        ...
        validate_international_home_network(events)
