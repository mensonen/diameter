...
# SCENARIO SELECTION
# ============================================================
SCENARIO_KEY = "mo_sms_offnet_home_country"

# Notes:
# - Other-Party-Id-Nature=1 for INTERNATIONAL was previously confirmed by your trace note.
# - For LOCAL/NATIONAL, the value below is a placeholder and should be verified in your environment.
# - IMS Number-Portability-Routing-Information examples from ICD include:
#   D083 MTN, D082 Vodacom, D081 Telkom, D084 Cell-C.
TEST_SCENARIOS = {
    "mo_sms_offnet_home_country": {
        "scenario_name": "MO SMS OFF-NET - SIM ON HOME NETWORK - HOME COUNTRY OTHER NETWORK",
        "description": "SIM on Home Network; destination is Home Country Not Home Network",
        "session_suffix": "sms-offnet-home-country",
        ...
        "type": "Local",
        "b_party_msisdn": "356803002227198",   # Replace with actual non-MTN domestic number
        "traffic_case": 20,

        "other_party_id_type": 0,
        "other_party_id_nature": 2,        # Placeholder for Local/National, verify in your env

        "enable_ims_information": True,
        "ims_node_functionality": 0,
        "ims_number_portability_routing_info": "D082",  # Replace if your non-MTN operator differs

        "enable_tgpp_mcc_mnc_avps": True,
        "tgpp_imsi_mcc_mnc": "65510",      # Sender MCCMNC
        "tgpp_ggsn_mcc_mnc": "65512",      # Destination MCCMNC expected charging dimension
        "tgpp_sgsn_mcc_mnc": "65510",      # SIM still on home network

        "expected_destination_mccmnc": "65512",
    },
}
...
def validate_offnet_home_country(events):
    print_banner("VERIFY OFF-NET HOME COUNTRY CHARGING")
    ...
    print(f"Expected Dest MCCMNC        : {CFG['expected_destination_mccmnc']}")

    if str(ggsn_mccmnc) != str(CFG["expected_destination_mccmnc"]):
        raise AssertionError(
            f"Expected destination MCCMNC {CFG['expected_destination_mccmnc']}, got {ggsn_mccmnc}"
        )

    print("Charging Verification PASSED")


def validate_kinesis(events):
    if SCENARIO_KEY == "mo_sms_offnet_home_country":
        validate_offnet_home_country(events)
    else:
        raise AssertionError(f"No validator implemented for scenario {SCENARIO_KEY}")
...
def main():
    ...
    try:
        with socket.create_connection((SERVER_FQDN, SERVER_PORT), timeout=10) as sock:
            ...
            print("Precondition only - assumed completed externally")

            print("\nSTEP 2: Send MO SMS to off-net domestic number")
