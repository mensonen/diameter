...
# SCENARIO SELECTION
# ============================================================
SCENARIO_KEY = "mo_sms_local_roaming_same_country"

TEST_SCENARIOS = {
    "mo_sms_local_roaming_same_country": {
        "scenario_name": "MO SMS LOCAL ROAMING - SIM IN ROAMING COUNTRY - DESTINATION SAME ROAMING COUNTRY",
        "description": "SIM in Roaming Country; destination in Same Roaming Country",
        "session_suffix": "sms-local-roaming-same-country",

        "sender_operator": "MTN",
        "sender_mcc": "404",
        "sender_mnc": "10",

        "receiver_operator": "NON_MTN",
        "receiver_mcc": "404",
        "receiver_mnc": "11",

        "type": "Roaming",
        "b_party_msisdn": "919811223344",   # Replace with actual visited-country local number
        "traffic_case": 20,

        "other_party_id_type": 0,
        "other_party_id_nature": 2,         # Local/National placeholder, verify in env

        "enable_ims_information": True,
        "ims_node_functionality": 0,
        "ims_number_portability_routing_info": "D084",  # Replace if your routing map differs

        "enable_tgpp_mcc_mnc_avps": True,
        "tgpp_imsi_mcc_mnc": "40410",       # Sender roaming MCCMNC
        "tgpp_ggsn_mcc_mnc": "40411",       # Destination MCCMNC in same roaming country
        "tgpp_sgsn_mcc_mnc": "40410",       # Serving network in roaming country

        "expected_classification": "MOSMS Local SMS",
        "expected_result": "YES",
    },
}
...
def validate_local_roaming_same_country(events):
    print_banner("VERIFY LOCAL ROAMING SMS")
    ...
    classification = deep_find_first(
        found_event,
        {"classification", "charging_classification", "service_classification"}
    )
    result_value = deep_find_first(
        found_event,
        {"result", "classified", "match", "classification_result"}
    )

    print(f"Expected Classification     : {CFG['expected_classification']}")
    print(f"Expected Result             : {CFG['expected_result']}")
    print(f"Kinesis Classification      : {classification}")
    print(f"Kinesis Result              : {result_value}")

    if classification is not None and str(classification) != str(CFG["expected_classification"]):
        raise AssertionError(
            f"Expected classification {CFG['expected_classification']}, got {classification}"
        )

    if result_value is not None and str(result_value).upper() != str(CFG["expected_result"]).upper():
        raise AssertionError(
            f"Expected result {CFG['expected_result']}, got {result_value}"
        )
...
def main():
    ...
    try:
        with socket.create_connection((SERVER_FQDN, SERVER_PORT), timeout=10) as sock:
            ...
            print("Precondition only - scenario assumes subscriber is active in roaming country")

            print("\nSTEP 2: Send MO SMS to same roaming country")
        ...
        validate_local_roaming_same_country(events)
