...
def normalize_kinesis_value(name, value):
    octet_string_names = {
        "TGPP-Charging-Id",
        "3GPP-Charging-Id",
        "3GPP-MS-TimeZone",
    ...
    if isinstance(value, str) and name in octet_string_names:
        try:
            return value.encode("latin1").hex().upper()
        except Exception:
            return value
...
def normalize_kinesis_event(event):
    cloned = json.loads(json.dumps(event, default=str))
    for avp in cloned.get("avps", []):
        avp_name = avp.get("name")
        avp["value"] = normalize_kinesis_value(avp_name, avp.get("value"))
        if isinstance(avp.get("value"), list):
            for child in avp["value"]:
                if isinstance(child, dict):
                    child_name = child.get("name")
                    child["value"] = normalize_kinesis_value(child_name, child.get("value"))
    return cloned
...
def main():
    ...
    print(f"Charging-Id                 : {CHARGING_ID_HEX.upper()} ({int.from_bytes(CHARGING_ID_BYTES, 'big')})")
    ...
    print(f"3GPP-IMSI-MCC-MNC           : {IMSI_MCC_MNC}")
    print(f"3GPP-GGSN-MCC-MNC           : {GGSN_MCC_MNC}")
    print(f"3GPP-SGSN-MCC-MNC           : {SGSN_MCC_MNC}")
    print(f"3GPP-NSAPI                  : {bytes_to_hex_upper(NSAPI)}")
    print(f"3GPP-MS-TimeZone            : {bytes_to_hex_upper(MS_TIMEZONE)}")
    print(f"3GPP-User-Location-Info     : {bytes_to_hex_upper(USER_LOCATION_INFO)}")
    print(f"3GPP-RAT-Type               : {bytes_to_hex_upper(RAT_TYPE)}")
    print(f"QCI                         : {QCI}")
    ...
    try:
        ...
        with socket.create_connection((SERVER_FQDN, SERVER_PORT), timeout=10) as sock:
            ...
            print(f"Charging-Id Sent            : {CHARGING_ID_HEX.upper()}")
            print(f"MS-TimeZone Sent            : {bytes_to_hex_upper(MS_TIMEZONE)}")
            print(f"User-Location-Info Sent     : {bytes_to_hex_upper(USER_LOCATION_INFO)}")
            print(f"RAT-Type Sent               : {bytes_to_hex_upper(RAT_TYPE)}")
