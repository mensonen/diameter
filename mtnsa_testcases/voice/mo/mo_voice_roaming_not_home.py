...
SCENARIO_KEY = "mo_voice_international_roaming_third_country"

TEST_SCENARIOS = {
    "mo_voice_international_roaming_third_country": {
        "scenario_name": "MO VOICE INTERNATIONAL ROAMING - SIM IN ROAMING COUNTRY - DESTINATION NOT HOME COUNTRY",
        "description": "SIM in Roaming Country; destination is Not Home Country",
        "session_suffix": "voice-international-roaming-third-country",
        ...
        # Update to real third-country destination
        "b_party_msisdn": "8801712345678",
        ...
        "requested_party_address": "8801712345678",
        "called_party_address": None,

        # Must be absent for international B-number
        ...
        "expected_source_charging_dimension": "Roaming MCCMNC",
        "expected_roaming_mcc": "404",
        "expected_roaming_mnc": "11",
        "expected_roaming_operator": "VODAFONE-IN",

        # Optional downstream business field
        "expected_additional_b_leg": "MOVOICE International",
    }
}
...
def print_field(label, value, width=36):
    if is_present(value):
...
def validate_source_dimension(event, expected_method):
    actual_dim = (
        event.get("source_charging_dimension")
        or event.get("charging_dimension")
        or event.get("roaming_charging_dimension")
    )
    actual_mcc = event.get("source_mcc") or event.get("roaming_mcc")
    actual_mnc = event.get("source_mnc") or event.get("roaming_mnc")
    actual_operator = event.get("source_operator") or event.get("roaming_operator")
    actual_b_leg = event.get("additional_b_leg") or event.get("b_leg_type") or event.get("additional_bleg")

    print_field(f"{expected_method} Source Charging Dimension", actual_dim, width=46)
    print_field(f"{expected_method} Source MCC", actual_mcc, width=46)
    print_field(f"{expected_method} Source MNC", actual_mnc, width=46)
    print_field(f"{expected_method} Source Operator", actual_operator, width=46)
    print_field(f"{expected_method} Additional B-Leg", actual_b_leg, width=46)

    if not (actual_dim is None and actual_mcc is None and actual_mnc is None and actual_operator is None):
        if str(actual_dim) != str(CFG["expected_source_charging_dimension"]):
            raise AssertionError(
                f"{expected_method}: expected source charging dimension {CFG['expected_source_charging_dimension']}, got {actual_dim}"
            )
        if str(actual_mcc) != str(CFG["expected_roaming_mcc"]):
            raise AssertionError(f"{expected_method}: expected source/roaming mcc {CFG['expected_roaming_mcc']}, got {actual_mcc}")
        if str(actual_mnc) != str(CFG["expected_roaming_mnc"]):
            raise AssertionError(f"{expected_method}: expected source/roaming mnc {CFG['expected_roaming_mnc']}, got {actual_mnc}")
        if str(actual_operator).upper() != str(CFG["expected_roaming_operator"]).upper():
            raise AssertionError(
                f"{expected_method}: expected source/roaming operator {CFG['expected_roaming_operator']}, got {actual_operator}"
            )

    if actual_b_leg is not None:
        if str(actual_b_leg).upper() != str(CFG["expected_additional_b_leg"]).upper():
            raise AssertionError(
                f"{expected_method}: expected additional B-leg {CFG['expected_additional_b_leg']}, got {actual_b_leg}"
            )
...
def validate_common_event_fields(event, expected_method):
    ...
    print_field(f"{expected_method} Method", method, width=46)
    print_field(f"{expected_method} Decision-Code", code, width=46)
    print_field(f"{expected_method} Service-Type", service_type, width=46)
    print_field(f"{expected_method} Granted-Voice-Sec", decision.get("granted_voice_seconds"), width=46)
    print_field(f"{expected_method} Granted-Data-Octets", decision.get("granted_data_octets"), width=46)
    ...
    print_field(f"{expected_method} Calling-Party", calling_party, width=46)
    print_field(f"{expected_method} Requested-Party", requested_party, width=46)
    print_field(f"{expected_method} Called-Party", called_party, width=46)
    print_field(f"{expected_method} Routing-Info", rn, width=46)
    print_field(f"{expected_method} Node-Functionality", node_functionality, width=46)
    print_field(f"{expected_method} Role-Of-Node", role_of_node, width=46)
    print_field(f"{expected_method} 3GPP-MS-TimeZone", bytes_to_hex_upper(ms_timezone), width=46)
    print_field(f"{expected_method} Bearer-Capability", bytes_to_hex_upper(bearer_capability), width=46)
    print_field(f"{expected_method} Net-Call-Ref-No", bytes_to_hex_upper(ncrn), width=46)
    print_field(f"{expected_method} MSC-Address", bytes_to_hex_upper(msc_address), width=46)
    print_field(f"{expected_method} VLR-Number", bytes_to_hex_upper(vlr_number), width=46)
    ...
    if rn not in (None, "", "null"):
        raise AssertionError(f"{expected_method}: Routing-Info should be absent for international B-number")
    ...
    validate_source_dimension(event, expected_method)
...
def validate_initial_event(event):
    ...
    print_field("INITIAL_REQUEST Requested CC-Time", requested_cc_time, width=46)
    print_field("INITIAL_REQUEST Used CC-Time", used_cc_time, width=46)
...
def validate_update_event(event):
    ...
    print_field("UPDATE_REQUEST Requested CC-Time", requested_cc_time, width=46)
    print_field("UPDATE_REQUEST Used CC-Time", used_cc_time, width=46)
...
def validate_termination_event(event):
    ...
    print_field("TERMINATION_REQUEST Requested CC-Time", requested_cc_time, width=46)
    print_field("TERMINATION_REQUEST Used CC-Time", used_cc_time, width=46)
...
def print_scenario_header():
    ...
    print_field("Expected Source Dimension", CFG["expected_source_charging_dimension"])
    ...
    print_field("Expected Additional B-Leg", CFG["expected_additional_b_leg"])
...
def main():
    ...
    try:
        ...
        with socket.create_connection((connect_host, SERVER_PORT), timeout=10) as sock:
            ...
            print_step("STEP 2: Initiate international MO call to third-country destination")
