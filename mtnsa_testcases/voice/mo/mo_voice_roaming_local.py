...
SCENARIO_KEY = "mo_voice_national_roaming_roamingcountry_samecountry"

TEST_SCENARIOS = {
    "mo_voice_national_roaming_roamingcountry_samecountry": {
        "scenario_name": "MO VOICE NATIONAL ROAMING - SIM REGISTERED ON ROAMING COUNTRY - CALL TO SAME ROAMING COUNTRY",
        "description": "SIM registered on roaming Country; call to same roaming Country",
        "session_suffix": "voice-national-roaming-roamingcountry-samecountry",

        "sender_operator": "MTN",
        "sender_mcc": "404",
        "sender_mnc": "10",

        # Update as per real roaming country/operator under test
        "receiver_operator": "VODAFONE-IN",
        "receiver_mcc": "404",
        "receiver_mnc": "11",

        "type": "national-roaming",

        "a_party_msisdn": "279603002227198",
        "a_party_imsi": "655103704646780",

        # Update to roaming-country national destination
        ...
        # Update to the routing number used for the national destination in the roaming country
        "number_portability_routing_information": "D40411",
        ...
        # Optional business validations if Kinesis exposes these fields
        "expected_destination_charging_dimension": "Destination MCCMNC",
        "expected_destination_mcc": "404",
        "expected_destination_mnc": "11",
        "expected_destination_operator": "VODAFONE-IN",
    }
}
...
def validate_common_event_fields(event, expected_method, expected_cc_request_type, expected_cc_request_number):
    ...
    if CFG.get("number_portability_routing_information") and str(rn) != str(CFG["number_portability_routing_information"]):
        raise AssertionError(f"{expected_method}: Number-Portability-Routing-Information mismatch")
...
def main():
    ...
    try:
        ...
        with socket.create_connection((connect_host, SERVER_PORT), timeout=10) as sock:
            ...
            print_step("STEP 1: Register SIM on Roaming Network")
            print("Precondition only - subscriber is attached on roaming network")

            print_step("STEP 2: Initiate MO call from roaming country to same roaming country")
