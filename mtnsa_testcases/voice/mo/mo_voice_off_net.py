...
# SCENARIO
# ============================================================
SCENARIO_KEY = "mo_voice_offnet_home_homecountry_not_home_network"

TEST_SCENARIOS = {
    "mo_voice_offnet_home_homecountry_not_home_network": {
        "scenario_name": "MO VOICE OFF-NET - SIM ON HOME NETWORK - DESTINATION HOME COUNTRY NOT HOME NETWORK",
        "description": "SIM on Home Network; destination is Home Country Not Home Network",
        "session_suffix": "voice-offnet-home-domestic-offnet",

        "sender_operator": "MTN",
        "sender_mcc": "655",
        "sender_mnc": "10",

        "receiver_operator": "VODACOM",
        "receiver_mcc": "655",
        "receiver_mnc": "01",
        ...
        "requested_party_address": "27821234567",
        "called_party_address": None,
        "number_portability_routing_information": "D082",
        ...
        "expected_destination_charging_dimension": "Destination MCCMNC",
        "expected_destination_mcc": "655",
        "expected_destination_mnc": "01",
        "expected_destination_operator": "VODACOM",
    }
}
...
def print_field(label, value, width=28):
    if is_present(value):
...
def build_voice_ccr(request_type: int, request_number: int, req_time: int = None, used_time: int = None) -> bytes:
    ...
    if CFG.get("requested_party_address"):
        ims_children.append(
            Avp.new(
                AVP_3GPP_REQUESTED_PARTY_ADDRESS,
                constants.VENDOR_TGPP,
                value=CFG["requested_party_address"]
            )
        )
    ...
    if CFG.get("called_party_address"):
        ims_children.append(
            Avp.new(
                AVP_3GPP_CALLED_PARTY_ADDRESS,
                constants.VENDOR_TGPP,
                value=CFG["called_party_address"]
            )
        )
    ...
    ps_children = []
    if CFG.get("tgpp_ms_timezone") is not None:
        ps_children.append(
            Avp.new(
                AVP_3GPP_MS_TIMEZONE,
                constants.VENDOR_TGPP,
                value=CFG["tgpp_ms_timezone"]
            )
        )
    ...
    vcs_children = []
    if CFG.get("bearer_capability") is not None:
        vcs_children.append(
            Avp.new(
                AVP_3GPP_BEARER_CAPABILITY,
                constants.VENDOR_TGPP,
                value=CFG["bearer_capability"]
            )
        )

    if CFG.get("network_call_reference_number") is not None:
        vcs_children.append(
            Avp.new(
                AVP_3GPP_NETWORK_CALL_REFERENCE_NUMBER,
                constants.VENDOR_TGPP,
                value=CFG["network_call_reference_number"]
            )
        )

    if CFG.get("msc_address") is not None:
        vcs_children.append(
            Avp.new(
                AVP_3GPP_MSC_ADDRESS,
                constants.VENDOR_TGPP,
                value=CFG["msc_address"]
            )
        )

    if CFG.get("vlr_number") is not None:
        vcs_children.append(
            Avp.new(
                AVP_3GPP_VLR_NUMBER,
                constants.VENDOR_TGPP,
                value=CFG["vlr_number"]
            )
        )
...
def get_nested_service_info_value(avps, parent_name, child_name, aliases=None):
    svc_info = get_service_information_group(avps)
    if not svc_info:
        return None
...
def validate_destination_dimension(event, expected_method):
    expected_dim = CFG.get("expected_destination_charging_dimension")
    expected_mcc = CFG.get("expected_destination_mcc")
    expected_mnc = CFG.get("expected_destination_mnc")
    expected_operator = CFG.get("expected_destination_operator")

    actual_dim = event.get("charging_dimension") or event.get("destination_charging_dimension")
    actual_mcc = event.get("destination_mcc")
    actual_mnc = event.get("destination_mnc")
    actual_operator = event.get("destination_operator")

    print_field(f"{expected_method} Destination Dimension", actual_dim, width=40)
    print_field(f"{expected_method} Destination MCC", actual_mcc, width=40)
    print_field(f"{expected_method} Destination MNC", actual_mnc, width=40)
    print_field(f"{expected_method} Destination Operator", actual_operator, width=40)
    ...
    if expected_dim and actual_dim != expected_dim:
        raise AssertionError(
            f"{expected_method}: expected destination charging dimension {expected_dim}, got {actual_dim}"
        )

    if expected_mcc and str(actual_mcc) != str(expected_mcc):
        raise AssertionError(f"{expected_method}: expected destination_mcc {expected_mcc}, got {actual_mcc}")

    if expected_mnc and str(actual_mnc) != str(expected_mnc):
        raise AssertionError(f"{expected_method}: expected destination_mnc {expected_mnc}, got {actual_mnc}")

    if expected_operator and str(actual_operator).upper() != str(expected_operator).upper():
        raise AssertionError(
            f"{expected_method}: expected destination_operator {expected_operator}, got {actual_operator}"
        )
...
def validate_common_event_fields(event, expected_method, expected_cc_request_type, expected_cc_request_number):
    ...
    granted_voice_seconds = decision.get("granted_voice_seconds", 0)
    granted_data_octets = decision.get("granted_data_octets", 0)

    print_field(f"{expected_method} Method", method, width=40)
    print_field(f"{expected_method} Decision-Code", code, width=40)
    print_field(f"{expected_method} Service-Type", service_type, width=40)
    print_field(f"{expected_method} Granted-Voice-Sec", granted_voice_seconds, width=40)
    print_field(f"{expected_method} Granted-Data-Octets", granted_data_octets, width=40)
    ...
    if service_type != CFG["expected_service_type"]:
        raise AssertionError(
            f"{expected_method}: expected service type {CFG['expected_service_type']}, got {service_type}"
        )
    ...
    if str(service_context_id) != str(CFG["service_context_id"]):
        raise AssertionError(
            f"{expected_method}: expected Service-Context-Id {CFG['service_context_id']}, got {service_context_id}"
        )

    if int(cc_request_type) != int(expected_cc_request_type):
        raise AssertionError(
            f"{expected_method}: expected CC-Request-Type {expected_cc_request_type}, got {cc_request_type}"
        )

    if int(cc_request_number) != int(expected_cc_request_number):
        raise AssertionError(
            f"{expected_method}: expected CC-Request-Number {expected_cc_request_number}, got {cc_request_number}"
        )

    imsi = get_subscription_value(avps, "END_USER_IMSI")
    e164 = get_subscription_value(avps, "END_USER_E164")

    if str(imsi) != str(CFG["a_party_imsi"]):
        raise AssertionError(f"{expected_method}: expected IMSI {CFG['a_party_imsi']}, got {imsi}")

    if str(e164) != str(CFG["a_party_msisdn"]):
        raise AssertionError(f"{expected_method}: expected E164 {CFG['a_party_msisdn']}, got {e164}")
    ...
    ms_timezone = get_nested_service_info_value(
        avps,
        "PS-Information",
        "3GPP-MS-TimeZone",
        aliases=["TGPP-MS-TimeZone"]
    )
    ...
    print_field(f"{expected_method} Calling-Party", calling_party, width=40)
    print_field(f"{expected_method} Requested-Party", requested_party, width=40)
    print_field(f"{expected_method} Called-Party", called_party, width=40)
    print_field(f"{expected_method} Routing-Info", rn, width=40)
    print_field(f"{expected_method} Node-Functionality", node_functionality, width=40)
    print_field(f"{expected_method} Role-Of-Node", role_of_node, width=40)
    print_field(f"{expected_method} 3GPP-MS-TimeZone", bytes_to_hex_upper(ms_timezone), width=40)
    print_field(f"{expected_method} Bearer-Capability", bytes_to_hex_upper(bearer_capability), width=40)
    print_field(f"{expected_method} Net-Call-Ref-No", bytes_to_hex_upper(ncrn), width=40)
    print_field(f"{expected_method} MSC-Address", bytes_to_hex_upper(msc_address), width=40)
    print_field(f"{expected_method} VLR-Number", bytes_to_hex_upper(vlr_number), width=40)

    if str(calling_party) != str(CFG["calling_party_address"]):
        raise AssertionError(
            f"{expected_method}: expected Calling-Party-Address {CFG['calling_party_address']}, got {calling_party}"
        )

    if str(requested_party) != str(CFG["requested_party_address"]):
        raise AssertionError(
            f"{expected_method}: expected Requested-Party-Address {CFG['requested_party_address']}, got {requested_party}"
        )

    if CFG.get("called_party_address") is None:
        if called_party not in (None, "", "null"):
            raise AssertionError(
                f"{expected_method}: expected Called-Party-Address to be absent, got {called_party}"
            )
    else:
        if str(called_party) != str(CFG["called_party_address"]):
            raise AssertionError(
                f"{expected_method}: expected Called-Party-Address {CFG['called_party_address']}, got {called_party}"
            )

    if str(rn) != str(CFG["number_portability_routing_information"]):
        raise AssertionError(
            f"{expected_method}: expected Number-Portability-Routing-Information "
            f"{CFG['number_portability_routing_information']}, got {rn}"
        )

    if int(node_functionality) != int(CFG["node_functionality"]):
        raise AssertionError(
            f"{expected_method}: expected Node-Functionality {CFG['node_functionality']}, got {node_functionality}"
        )

    if int(role_of_node) != int(CFG["role_of_node"]):
        raise AssertionError(
            f"{expected_method}: expected Role-Of-Node {CFG['role_of_node']}, got {role_of_node}"
        )
    ...
    if int(service_identifier) != int(CFG["service_identifier"]):
        raise AssertionError(
            f"{expected_method}: expected Service-Identifier {CFG['service_identifier']}, got {service_identifier}"
        )

    validate_destination_dimension(event, expected_method)

    return avps, decision


def validate_initial_event(event):
    avps, decision = validate_common_event_fields(
        event=event,
        expected_method="INITIAL_REQUEST",
        expected_cc_request_type=1,
        expected_cc_request_number=0,
    )

    requested_cc_time = get_mscc_cc_time(avps, "Requested-Service-Unit")
    used_cc_time = get_mscc_cc_time(avps, "Used-Service-Unit")

    print_field("INITIAL_REQUEST Requested CC-Time", requested_cc_time, width=40)
    print_field("INITIAL_REQUEST Used CC-Time", used_cc_time, width=40)
    ...
    if decision.get("granted_voice_seconds") != CFG["expected_initial_grant"]:
        raise AssertionError(
            ...
            f"got {decision.get('granted_voice_seconds')}"
        )
...
def validate_update_event(event):
    avps, decision = validate_common_event_fields(
        event=event,
        expected_method="UPDATE_REQUEST",
        expected_cc_request_type=2,
        expected_cc_request_number=1,
    )

    requested_cc_time = get_mscc_cc_time(avps, "Requested-Service-Unit")
    used_cc_time = get_mscc_cc_time(avps, "Used-Service-Unit")

    print_field("UPDATE_REQUEST Requested CC-Time", requested_cc_time, width=40)
    print_field("UPDATE_REQUEST Used CC-Time", used_cc_time, width=40)
...
def validate_termination_event(event):
    avps, decision = validate_common_event_fields(
        event=event,
        expected_method="TERMINATION_REQUEST",
        expected_cc_request_type=3,
        expected_cc_request_number=2,
    )

    requested_cc_time = get_mscc_cc_time(avps, "Requested-Service-Unit")
    used_cc_time = get_mscc_cc_time(avps, "Used-Service-Unit")

    print_field("TERMINATION_REQUEST Requested CC-Time", requested_cc_time, width=40)
    print_field("TERMINATION_REQUEST Used CC-Time", used_cc_time, width=40)

    if requested_cc_time not in (None, 0, "0"):
        raise AssertionError(
            f"TERMINATION_REQUEST: expected no Requested-Service-Unit/CC-Time, got {requested_cc_time}"
        )
    ...
    if termination_grant not in (0, None):
        raise AssertionError(
            f"TERMINATION_REQUEST: expected granted voice seconds 0 or None, got {termination_grant}"
        )
...
def main():
    ...
    print_field("Normalized B-Party MSISDN", CFG["normalized_b_party_msisdn"])
    ...
    print_field("Expected Charging Dimension", CFG["expected_destination_charging_dimension"])
    print_field("Expected Destination MCC", CFG["expected_destination_mcc"])
    print_field("Expected Destination MNC", CFG["expected_destination_mnc"])
    print_field("Expected Destination Oper", CFG["expected_destination_operator"])
    ...
    try:
        ...
        with socket.create_connection((connect_host, SERVER_PORT), timeout=10) as sock:
            ...
            print("\nSTEP 2: Initiate MO call to domestic off-net number")
            ...
            print("\nSTEP 7: Verify OCS charging output")
