...
# SCENARIO
# ============================================================
SCENARIO_KEY = "mo_voice_onnet_home_home_normalized"

TEST_SCENARIOS = {
    "mo_voice_onnet_home_home_normalized": {
        "scenario_name": "MO VOICE ON-NET NORMALIZED - SIM ON HOME NETWORK - DESTINATION ON HOME NETWORK",
        "description": "SIM on Home Network; destination on Home Network; MO number normalization applied",
        "session_suffix": "voice-onnet-home-home-normalized",
        ...
        "b_party_msisdn": "276553334444",
        "normalized_b_party_msisdn": "276553300000",
        ...
        "requested_party_address": "276553334444",
        "called_party_address": "276553300000",
...
def validate_common_event_fields(event, expected_method, expected_cc_request_type, expected_cc_request_number):
    ...
    print(f"{expected_method} Requested-Party    : {requested_party}")
    print(f"{expected_method} Called-Party       : {called_party}")
    print(f"{expected_method} Routing-Info       : {rn}")
...
def main():
    ...
    print(f"A-Party MSISDN              : {CFG['a_party_msisdn']}")
    print(f"A-Party IMSI                : {CFG['a_party_imsi']}")
    print(f"B-Party MSISDN              : {CFG['b_party_msisdn']}")
    print(f"Normalized B-Party MSISDN   : {CFG['normalized_b_party_msisdn']}")
    print(f"Service-Identifier          : {CFG['service_identifier']}")
    print(f"Node-Functionality          : {CFG['node_functionality']}")
    print(f"Role-Of-Node                : {CFG['role_of_node']}")
    print(f"Calling-Party-Address       : {CFG['calling_party_address']}")
    print(f"Requested-Party-Address     : {CFG['requested_party_address']}")
    print(f"Called-Party-Address        : {CFG['called_party_address']}")
    print(f"MNP Routing Information     : {CFG['number_portability_routing_information']}")
    print(f"3GPP-MS-TimeZone            : {display_value(CFG['tgpp_ms_timezone'])}")
    print(f"Bearer-Capability           : {display_value(CFG['bearer_capability'])}")
    print(f"Network-Call-Reference-No   : {display_value(CFG['network_call_reference_number'])}")
    ...
    try:
        ...
        with socket.create_connection((connect_host, SERVER_PORT), timeout=10) as sock:
            ...
            print("\nSTEP 2: Initiate MO voice call to normalized on-net number")
            ...
            print("\nSTEP 3: Keep call active for billable duration")
