...
SCENARIO_KEY = "mt_voice_home_no_charge"

TEST_SCENARIOS = {
    "mt_voice_home_no_charge": {
        "scenario_name": "MT VOICE HOME - CALLED SIM ON HOME NETWORK - SOURCE FROM VALID SOURCE TYPE",
        "description": "Called SIM on Home Network; source from Home Network or Home Country Not Home Network or Not Home Country",
        "session_suffix": "mt-voice-home-no-charge",

        "called_operator": "MTN",
        "called_mcc": "655",
        "called_mnc": "10",

        # Set the active source case under test
        "source_case": "HOME_COUNTRY_NOT_HOME_NETWORK",

        "source_operator": "VODACOM",
        "source_mcc": "655",
        "source_mnc": "01",

        "type": "mt-home",

        # B-party / called subscriber
        "called_party_msisdn": "279603002227198",
        "called_party_imsi": "655103704646780",

        # A-party / source caller
        "calling_party_msisdn": "27821234567",
        ...
        "calling_party_address": "27821234567",
        ...
        # Business expectation from downstream event model
...
def build_mt_voice_ccr(request_type, request_number, req_time=None, used_time=None):
    ...
    # Subscription-Id is still the subscriber being evaluated by charging
...
def validate_no_charge_business(event, expected_method):
    ...
    print_field(f"{expected_method} Business Result", actual_result, width=44)
    print_field(f"{expected_method} Charge Amount", actual_charge_amount, width=44)
    print_field(f"{expected_method} Chargeable", actual_chargeable, width=44)
...
def validate_common_event_fields(event, expected_method):
    ...
    print_field(f"{expected_method} Method", method, width=44)
    print_field(f"{expected_method} Decision-Code", code, width=44)
    print_field(f"{expected_method} Service-Type", service_type, width=44)
    print_field(f"{expected_method} Granted-Voice-Sec", decision.get("granted_voice_seconds"), width=44)
    ...
    print_field(f"{expected_method} Calling-Party", calling_party, width=44)
    print_field(f"{expected_method} Called-Party", called_party, width=44)
    print_field(f"{expected_method} Node-Functionality", node_functionality, width=44)
    print_field(f"{expected_method} Role-Of-Node", role_of_node, width=44)
...
def validate_initial_event(event):
    ...
    print_field("INITIAL_REQUEST Requested CC-Time", requested_cc_time, width=44)
...
def validate_update_event(event):
    ...
    print_field("UPDATE_REQUEST Requested CC-Time", requested_cc_time, width=44)
    print_field("UPDATE_REQUEST Used CC-Time", used_cc_time, width=44)
...
def validate_termination_event(event):
    ...
    print_field("TERMINATION_REQUEST Used CC-Time", used_cc_time, width=44)
...
def validate_voice_kinesis(events):
    print_banner("VERIFY MT HOME FROM KINESIS")
    if not events:
    ...
    print("Kinesis MT home verification PASSED")
...
def print_scenario_header():
    ...
    print_field("Source Case", CFG["source_case"])
...
def main():
    ...
    try:
        try:
            ...
            connect_host = addrinfo[0][4][0]
        except socket.gaierror:
            connect_host = "172.30.12.10"
        ...
        with socket.create_connection((connect_host, SERVER_PORT), timeout=10) as sock:
            ...
            print_step("STEP 2: Trigger MT voice call from selected valid source type")
            ...
            print_step("STEP 5: Verify charging on destination subscriber")
        ...
        print_step("STEP 6: Get expected data from Kinesis")
