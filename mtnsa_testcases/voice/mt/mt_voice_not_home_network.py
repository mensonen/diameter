...
SCENARIO_KEY = "mt_voice_home_country_not_home_network_no_charge"

TEST_SCENARIOS = {
    "mt_voice_home_country_not_home_network_no_charge": {
        "scenario_name": "MT VOICE - CALLED SIM ON HOME COUNTRY NOT HOME NETWORK",
        "description": "Called SIM on Home Country Not Home Network",
        "session_suffix": "mt-voice-home-country-not-home-network-no-charge",

        "called_operator": "MTN",
        "called_mcc": "655",
        "called_mnc": "10",

        "serving_operator": "VODACOM",
        "serving_mcc": "655",
        "serving_mnc": "01",

        "source_operator": "CELLC",
        "source_mcc": "655",
        "source_mnc": "07",

        "type": "mt-national-roaming",

        "called_party_msisdn": "279603002227198",
        "called_party_imsi": "655103704646780",
        "calling_party_msisdn": "27841234567",
        ...
        "calling_party_address": "27841234567",
...
def print_field(label, value, width=40):
    if is_present(value):
...
def validate_no_charge_business(event, expected_method):
    ...
    print_field(f"{expected_method} Business Result", actual_result, width=48)
    print_field(f"{expected_method} Charge Amount", actual_charge_amount, width=48)
    print_field(f"{expected_method} Chargeable", actual_chargeable, width=48)

    if actual_result is not None and str(actual_result).upper() != str(CFG["expected_result"]).upper():
        raise AssertionError(f"{expected_method}: expected result {CFG['expected_result']}, got {actual_result}")
    if actual_charge_amount is not None and float(actual_charge_amount) != float(CFG["expected_charge_amount"]):
        raise AssertionError(f"{expected_method}: expected charge_amount {CFG['expected_charge_amount']}, got {actual_charge_amount}")
    if actual_chargeable is not None and bool(actual_chargeable) != bool(CFG["expected_chargeable"]):
...
def validate_common_event_fields(event, expected_method):
    ...
    print_field(f"{expected_method} Method", method, width=48)
    print_field(f"{expected_method} Decision-Code", code, width=48)
    print_field(f"{expected_method} Service-Type", service_type, width=48)
    print_field(f"{expected_method} Granted-Voice-Sec", decision.get("granted_voice_seconds"), width=48)
    ...
    print_field(f"{expected_method} Calling-Party", calling_party, width=48)
    print_field(f"{expected_method} Called-Party", called_party, width=48)
    print_field(f"{expected_method} Node-Functionality", node_functionality, width=48)
    print_field(f"{expected_method} Role-Of-Node", role_of_node, width=48)
    print_field(f"{expected_method} 3GPP-MS-TimeZone", bytes_to_hex_upper(ms_timezone), width=48)
    print_field(f"{expected_method} Bearer-Capability", bytes_to_hex_upper(bearer_capability), width=48)
    print_field(f"{expected_method} Net-Call-Ref-No", bytes_to_hex_upper(ncrn), width=48)
    print_field(f"{expected_method} MSC-Address", bytes_to_hex_upper(msc_address), width=48)
    print_field(f"{expected_method} VLR-Number", bytes_to_hex_upper(vlr_number), width=48)
...
def validate_initial_event(event):
    ...
    print_field("INITIAL_REQUEST Requested CC-Time", requested_cc_time, width=48)
...
def validate_update_event(event):
    ...
    print_field("UPDATE_REQUEST Requested CC-Time", requested_cc_time, width=48)
    print_field("UPDATE_REQUEST Used CC-Time", used_cc_time, width=48)
...
def validate_termination_event(event):
    ...
    print_field("TERMINATION_REQUEST Used CC-Time", used_cc_time, width=48)
...
def validate_voice_kinesis(events):
    print_banner("VERIFY MT VOICE NO CHARGE FROM KINESIS")
    if not events:
    ...
    print("Kinesis MT no-charge verification PASSED")
...
def main():
    ...
    try:
        ...
        with socket.create_connection((connect_host, SERVER_PORT), timeout=10) as sock:
            ...
            print_step("STEP 1: Register called SIM on national roaming network")
            print("Precondition only - called subscriber attached on national roaming network")

            print_step("STEP 2: Trigger MT call from another home network within the home country")
            ...
            #print("Diameter MT flow completed; no-charge business result will be checked in Kinesis")
        ...
        print(f"MT voice no-charge flow failed: {e}")
        raise
